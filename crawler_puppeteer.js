    ...
    for (let i = 0; i < Math.min(productLinks.length, maxItems); i++) {
        ...
        try {
            ...
            const productDataRaw = await page.evaluate(() => {
                ...
                return { name, priceText, originalPriceText, rawImages, specs };
            });
            ...
            const productData = {
                ...
                description: "Sản phẩm được tự động thu thập thông tin.",
                shortDescription: "Sản phẩm chính hãng tại Đức Thịnh TechShop.",
                warrantyMonths: 12,
                createdAt: new Date(),
                updatedAt: new Date()
            };
...
async function main() {
    let collectionUrl = "https://gearvn.com/collections/loa-vi-tinh-gearvn";
    let categoryName = "Loa vi tính";
    let brandName = "GearVN";
    let maxProducts = 20;
